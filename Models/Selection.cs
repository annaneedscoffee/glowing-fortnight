...
namespace DiagramDesigner
{
    internal class Selection
    {
        ...
        public Selection(DiagramCanvas canvas)
        {
        ...
        {
            ClearSelection();
            CurrentSelection.AddRange(DiagramCanvas.Children.OfType<ISelectable>());
            CurrentSelection.ForEach(item => item.IsSelected = true);
        }

        internal List<IGroupable> GetGroupMembers(IGroupable item)
        {
            IEnumerable<IGroupable> list = DiagramCanvas.Children.OfType<IGroupable>();
